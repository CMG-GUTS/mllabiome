...
_PERCENT_METRICS = {'AUC', 'PR_AUC', 'nMCC', 'Accuracy', 'BalAcc', 'F1', 'F1w', 'Precision', 'Recall'}
_PERFORMANCE_METRIC_ORDER = ('AUC', 'PR_AUC', 'nMCC', 'Accuracy', 'BalAcc', 'F1w', 'Precision', 'Recall', 'Brier', 'Brier_multiclass', 'LogLoss')
_CALIBRATION_METRIC_ORDER = ('CalibrationInTheLarge', 'CalibrationIntercept', 'CalibrationSlope')
_METRIC_LABELS = {'AUC': 'ROC-AUC', 'PR_AUC': 'PR-AUC (AP)', 'nMCC': 'nMCC', 'Accuracy': 'Accuracy', 'BalAcc': 'Balanced accuracy', 'F1': 'F1', 'F1w': 'F1w', 'Precision': 'Precision', 'Recall': 'Recall', 'Brier': 'Brier score', 'Brier_multiclass': 'Multiclass Brier score', 'LogLoss': 'Log loss', 'CalibrationInTheLarge': 'Calibration-in-the-large', 'CalibrationIntercept': 'Calibration intercept', 'CalibrationSlope': 'Calibration slope'}
_ESTIMAND_LABELS = {'mean_repeat_pooled_oof': 'Pooled OOF across outer folds; mean across repeats', 'pooled_sample_weighted': 'Pooled OOF, sample-weighted', 'cohort_macro_equal_weight': 'Pooled OOF, equal-cohort weighting'}
...
def _wide_metric_table(performance: pd.DataFrame, metric_order: tuple[str, ...]) -> pd.DataFrame:
...
def _calibration_display(performance: pd.DataFrame) -> pd.DataFrame:
    if performance.empty or 'metric' not in performance.columns:
        return pd.DataFrame()
    calibration = performance[performance['metric'].astype(str).isin(_CALIBRATION_METRIC_ORDER)].copy()
...
def _coverage_display(coverage: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in coverage.iterrows():
        ...
        if reason and reason.lower() != 'nan':
            item['Reason'] = reason
        rows.append(item)
    order = ['Strategy', 'Subjects', 'Repeats', 'Held-out cohorts', 'Complete OOF coverage', 'Inference population', 'Status', 'Reason']
            ...
            for value in design.values():
                if isinstance(value, dict):
                    try:
                        ...
                        pass
        text = 'Pooled held-out estimates use a subject-cluster bootstrap' if repeats and max(repeats) == 1 else 'Pooled held-out estimates use subject-cluster and repeat resampling'
    if bootstrap:
...
def _probability_semantics_html(manifest: dict[str, Any]) -> str:
    ...
    for strategy, info in semantics.items():
        ...
        if isinstance(aggregations, list) and aggregations:
            detail = 'outer-fold aggregation: ' + ', '.join((str(x) for x in aggregations))
        ...
        omitted.append(f'<strong>{html.escape(str(strategy))}</strong> ({html.escape(detail)})')
    if not omitted:
        return ''
    return '<p>Proper scoring and calibration metrics are reported only for strategies with verified probabilistic outputs. Omitted: ' + '; '.join(omitted) + '.</p>'
...
def _contrast_display(contrasts: pd.DataFrame) -> pd.DataFrame:
    required = {'strategy_a', 'strategy_b', 'estimand', 'metric', 'is_selection_metric', 'estimate_a', 'estimate_b', 'advantage_a_over_b', 'advantage_ci_low', 'advantage_ci_high'}
    if contrasts.empty or not required.issubset(contrasts.columns):
        return pd.DataFrame()
    selection_metrics = set(contrasts.loc[contrasts['is_selection_metric'].fillna(False).astype(bool), 'metric'].astype(str))
    preferred = selection_metrics | {'AUC', 'PR_AUC', 'Brier', 'Brier_multiclass', 'LogLoss'}
    shown = contrasts[contrasts['metric'].astype(str).isin(preferred)].copy()
    if shown.empty:
        shown = contrasts.copy()
    estimands = [str(x) for x in shown['estimand'].dropna().astype(str).unique().tolist() if str(x)]
    show_estimand = len(set(estimands)) > 1
    rows: list[dict[str, Any]] = []
    for _, row in shown.iterrows():
        ...
        item: dict[str, Any] = {'Strategy A': str(row.get('strategy_a', '')), 'Strategy B': str(row.get('strategy_b', ''))}
        if show_estimand:
            item['Estimand'] = _ESTIMAND_LABELS.get(str(row.get('estimand', '')), str(row.get('estimand', '')))
        item.update({'Metric': metric_label, 'A': _format_contrast_value(row.get('estimate_a'), metric), 'B': _format_contrast_value(row.get('estimate_b'), metric), 'Advantage A over B (95% CI)': advantage_text, 'Interpretation': interpretation})
        rows.append(item)
    return pd.DataFrame(rows)

def _links_html(tables_dir: Path) -> str:
    names = [('strategy_oof_performance.tsv', 'Long-form OOF estimates'), ('strategy_oof_performance_table.tsv', 'Wide OOF table'), ('strategy_oof_calibration.tsv', 'Reliability-curve data'), ('strategy_oof_pairwise_contrasts.tsv', 'Paired OOF contrasts'), ('strategy_oof_statistics_manifest.json', 'OOF methods manifest')]
    links = []
    for filename, label in names:
        path = tables_dir / filename
        if path.exists():
            links.append(f'<a href="tables/{html.escape(filename)}">{html.escape(label)}</a>')
    if not links:
        return ''
...
def _replace_procedure_layout(text: str, report_dir: Path) -> str:
    ...
    grid_start = text.find('<div class="procedure-grid">', heading + len(_PROCEDURE_ANCHOR))
    if grid_start != -1 and (next_h2 == -1 or grid_start < next_h2):
        return text
    table_start = text.find('<div class="table-wrap">', heading + len(_PROCEDURE_ANCHOR))
    if table_start == -1:
        return text
...
def _insert_mpma_b_composition(text: str, report_dir: Path) -> str:
    ...
    if heading == -1:
        return text
    table_start = text.find('<div class="table-wrap">', heading + len(_PERFORMANCE_ANCHOR))
    if table_start == -1:
        return text
    table_end = text.find('</div>', table_start)
    if table_end == -1:
    ...
    table_end += len('</div>')
    section = '\n'.join(
        [_MPMA_B_SECTION_START, '<h3>Final MPMA-B specification</h3>', _html_table(table), _MPMA_B_SECTION_END]
    )
...
def _section_html(report_dir: Path) -> str:
    ...
    parts = [_SECTION_START, '<h2 id="oof-performance">Pooled out-of-fold performance</h2>', '<p>Performance is calculated from held-out predictions pooled across outer test folds. Discrimination and classification metrics are percentages; proper scoring rules and calibration statistics are shown on their natural scales.</p>', _methodology_html(manifest), _html_table(performance_table), _probability_semantics_html(manifest)]
    if not contrast_table.empty:
        parts.extend(['<h3 id="oof-contrasts">Paired strategy contrasts</h3>', '<p>Paired differences use matched held-out observations and the same resampling structure as the pooled estimates. Positive values favor Strategy A. Brackets denote 95% confidence intervals.</p>', _html_table(contrast_table)])
    if not calibration_table.empty:
        parts.extend(['<h3 id="oof-calibration-summary">Calibration</h3>', '<p>Ideal values are 0 for calibration-in-the-large and intercept, and 1 for slope.</p>', _html_table(calibration_table)])
    elif not calibration_curve.empty:
        parts.extend(['<h3 id="oof-calibration-summary">Calibration</h3>', '<p>One-vs-rest reliability data are available in <code>strategy_oof_calibration.tsv</code>.</p>'])
...
def _statistics_section_end(text: str) -> int:
    ...
    if start == -1:
        raise RuntimeError('Cannot verify report layout: the statistical-comparisons HTML anchor was not found. The base report template has changed and the integration must be updated explicitly.')
    search_from = start + len(_STATISTICS_ANCHOR)
    candidates = [pos for pos in (text.find('<h2 ', search_from), text.find('<h3 ', search_from)) if pos != -1]
    if candidates:
...
def _extract_compute_block(text: str) -> tuple[str, str]:
    ...
    if start == -1:
        raise RuntimeError('Cannot locate Computational resources. The base report template has changed and the integration must be updated explicitly.')
...
def enhance_html_report(report_dir: Path | str) -> bool:
    ...
    if marker not in text:
        raise RuntimeError('OOF insertion slot was lost while normalizing report layout')
...
def _terminal_oof_summary(report_dir: Path) -> None:
    ...
    if display.empty:
        return
    preferred = ['Strategy', 'Estimand', 'ROC-AUC', 'PR-AUC (AP)', 'nMCC', 'Brier score', 'Log loss']
...
def write_report(sweep: Any) -> dict[str, Path]:
    ...
    new_outputs = {'strategy_oof_performance': tables_dir / 'strategy_oof_performance.tsv', 'strategy_oof_performance_table': tables_dir / 'strategy_oof_performance_table.tsv', 'strategy_oof_calibration': tables_dir / 'strategy_oof_calibration.tsv', 'strategy_oof_pairwise_contrasts': tables_dir / 'strategy_oof_pairwise_contrasts.tsv', 'strategy_oof_statistics_manifest': tables_dir / 'strategy_oof_statistics_manifest.json'}
