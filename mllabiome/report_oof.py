...
def _probability_semantics_html(manifest: dict[str, Any]) -> str:
    ...
    for strategy, info in semantics.items():
        ...
        if isinstance(aggregations, list) and aggregations:
            detail = ", ".join(str(x) for x in aggregations) + " aggregation"
    ...
    return (
        "<p>Proper scoring and calibration metrics are reported only for strategies "
        "with verified probabilistic outputs. Omitted: " + "; ".join(omitted) + ".</p>"
    )
...
def _section_html(report_dir: Path) -> str:
    ...
    parts = [
        ...
        (
            "<p>Performance is calculated from held-out predictions pooled across outer "
            "test folds. Discrimination and classification metrics are percentages; "
            "proper scoring rules and calibration statistics are shown on their natural scales.</p>"
        ),
    ...
    if not contrast_table.empty:
        parts.extend(
            [
                ...
                (
                    "<p>Paired differences use matched held-out observations and the same "
                    "resampling structure as the pooled estimates. Positive values favor "
                    "Strategy A. Brackets denote 95% confidence intervals.</p>"
                ),
...
def _extract_compute_block(text: str) -> tuple[str, str]:
    ...
    if start == -1:
        raise RuntimeError(
            "Cannot locate Computational resources. The base report template has changed "
            "and the integration must be updated explicitly."
        )
