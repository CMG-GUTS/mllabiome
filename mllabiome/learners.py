...
def _learner_factory(item: Any) -> tuple[str, Callable[[], BaseEstimator]]:
    if isinstance(item, tuple):
        ...
        raise TypeError(f"Learner tuple for {name!r} must contain an estimator or factory.")
...
class FLAMLClassifier(BaseEstimator):
    ...
    def __init__(self, time_budget=600, metric="roc_auc", estimator_list=None, n_jobs=1, random_state=42, verbose=0, **kwargs):
...
def build_learner(name: str) -> BaseEstimator:
    ...
    if base in {"lr", "lr_l2", "lr_l2_bal", "logistic"}:
        return _logistic_regression(kind="l2", max_iter=2000, class_weight="balanced", random_state=42)
    if base in {"lr_l1", "lr_l1_bal"}:
        return _logistic_regression(kind="l1", max_iter=3000, class_weight="balanced", random_state=42)
    ...
    if base in {"rf", "rf_500_msl5"}:
        return RandomForestClassifier(n_estimators=500, min_samples_leaf=5, n_jobs=1, random_state=42)
    if base in {"rf_1000_msl5", "baseline_rf"}:
        return RandomForestClassifier(n_estimators=1000, min_samples_leaf=5, n_jobs=1, random_state=42)
    if base in {"rf_200", "rf_fast"}:
        return RandomForestClassifier(n_estimators=200, min_samples_leaf=2, n_jobs=1, random_state=42)
    if base in {"et", "extratrees"}:
        return ExtraTreesClassifier(n_estimators=500, min_samples_leaf=3, n_jobs=1, random_state=42)
    if base in {"histgb", "histgradientboosting"}:
        return HistGradientBoostingClassifier(random_state=42)
    if base in {"svc_rbf", "svc_rbf_bal"}:
        return SVC(kernel="rbf", C=1.0, gamma="scale", probability=True, class_weight="balanced", random_state=42)
    if base in {"calib_lsvc", "caliblsvc_c1_sig"}:
        return CalibratedClassifierCV(LinearSVC(C=1.0, class_weight="balanced", max_iter=5000, random_state=42), method="sigmoid", cv=3)
    ...
    if base in {"pa", "pa_default"}:
        return CalibratedClassifierCV(PassiveAggressiveClassifier(random_state=42), method="sigmoid", cv=3)
