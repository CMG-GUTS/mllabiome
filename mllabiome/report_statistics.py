...
_NONPROBABILITY_LEARNER_PREFIXES = (
    "ridge",
    "nearestcentroid",
    "siamcat",
)
...
def _learner_probability_semantics(learner: Any) -> tuple[bool, str]:
    ...
    if key.startswith(_NONPROBABILITY_LEARNER_PREFIXES):
        if key.startswith("ridge"):
            return (
                False,
                "RidgeClassifier predictions are decision scores transformed by sigmoid, not calibrated probabilities",
            )

        if key.startswith("nearestcentroid"):
            return (
                False,
                "NearestCentroid has no probability model; probability-like values are derived from hard predictions",
            )

        if key.startswith("siamcat"):
            return (
                False,
                "SIAMCAT scores may be passed through a logistic transform and are not guaranteed to be calibrated probabilities",
            )

    if key.startswith(_PROBABILITY_LEARNER_PREFIXES):
        return True, "estimator exposes a probability forecast"
    ...
    if str(strategy) == "MPMA-E":
        ...
        if invalid_aggs:
            return {
                ...
                "reason": (
                    "ensemble aggregation is not probability-preserving: "
                    + ", ".join(invalid_aggs)
                ),
...
def _canonical_pair_frame(
    ...
    if left_keys != right_keys:
        ...
        raise ValueError(
            f"{strategy_a} vs {strategy_b}: paired OOF coverage differs "
            f"({missing_from_b} observation(s) missing from {strategy_b}; "
            f"{missing_from_a} observation(s) missing from {strategy_a})"
        )
...
def _run_oof_statistics(
    ...
    for strategy, raw_predictions in frames.items():
        ...
        if selection_metric in _PROBABILITY_ONLY_METRICS and not bool(
            semantics.get("valid")
        ):
            raise ValueError(
                f"{strategy}: selection metric {selection_metric!r} requires probabilistic outputs, "
                f"but this strategy does not provide them ({semantics.get('reason', 'unverified probability semantics')})."
            )
