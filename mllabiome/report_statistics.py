...
def _strategy_prediction_frames(
    ...
    if "MPMA-B" in rows:
        frames["MPMA-B"] = _ensure_outer_split_key(
            fixed_strategy_predictions(root, "MPMA-B")
        )
    if "MPMA-E" in rows:
        try:
            frames["MPMA-E"] = _ensure_outer_split_key(
                fixed_strategy_predictions(root, "MPMA-E")
            )
    ...
    if not outer.empty and "config_id" in outer.columns:
        ...
        for strategy in ("AutoML", "Baseline RF", "SIAMCAT"):
            if strategy not in rows:
                continue
            config_id = str(rows[strategy].get("config_id", "")).strip()
            if not config_id:
                continue
...
def _strategy_selection_scope(strategy: str, row: dict[str, Any]) -> str:
    ...
    if strategy == "MPMA-E":
        return "final_selected_ensemble_specification"
    if selection_basis:
    ...
    *,
    ...
    advanced_manifest = {
        ...
        "selection_interpretation": {
            "MPMA-B": "Single final MPMA configuration selected from inner-validation results; all reported MPMA-B metrics use that config_id across outer held-out predictions.",
            "MPMA-E": "Single final ensemble specification; all reported MPMA-E metrics use the same members and aggregation across outer held-out predictions.",
